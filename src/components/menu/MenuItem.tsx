...
const MenuItems = ({ items, depthLevel }: any) => {
  ...
  return (
    <li
      className={`menu-items group w-full px-4 lg:px-0 lg:w-max ${
        depthLevel > 0 ? '!text-[14px] lg:min-w-[300px] !leading-3' : '!text-lg'
      }`}
      ref={ref}
    ...
    >
      {items.url && items.submenu ? (
        <>
          <button
            type='button'
            aria-haspopup='menu'
            className='flex justify-between hihi'
      ...
      {depthLevel === 0 && (
        <div className='bg-red-600 w-0 transition-all duration-300 lg:group-hover:w-[90%] left-1/2 -translate-x-1/2 h-0.5 top-full -translate-y-2 absolute'></div>
      )}
    </li>
  )
}
